...
<GridLayout rows="*,*,*,*">
    ...
    <GridLayout row="1" v-if="showEdit" class="details" :rows="colsOrRowsForTimes('rows')">
      <Label v-for="(val, key, index) in times" v-bind:key="'ed' + item.id + index" :row="index" :text="timeString(val)"></Label>
    </GridLayout>
